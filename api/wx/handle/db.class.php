...
class DB
{
	...
	function open($p_error="1" )
	{
		...
		{
			//数据库连接错误自动处理
			$this->_conn = @mysql_connect( $_CFG["DB"]['DB_HOST'], $_CFG["DB"]['DB_USER'], $_CFG["DB"]['DB_PWD'] ) or die( "系维升级中.." );
		}
		else
		{
			//数据库连接错误人工处理
			$this->_conn = @mysql_connect( $_CFG["DB"]['DB_HOST'], $_CFG["DB"]['DB_USER'], $_CFG["DB"]['DB_PWD'] );
			if(!$this->_conn)
			{
				return false;
			}
